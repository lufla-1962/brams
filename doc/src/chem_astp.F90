...
91     format(//,' Reading pressure gridded netCDF data',/,a,//)
       ...
       do i=1,nvars
           select case (trim(varName(i)))
               case (trim(WIND_U_VARNAME))
                   iFound=iFound+1
                   uVarN=i
               case (trim(WIND_V_VARNAME))
                   iFound=iFound+1
                   vVarN=i
               case (trim(TEMPERATURE_VARNAME))
                   iFound=iFound+1
                   tVarN=i
               case (trim(GEO_VARNAME))
                   iFound=iFound+1
                   zVarN=i
               case (trim(UR_VARNAME))
                   iFound=iFound+1
                   qVarN=i
           end select
       enddo
...
!  recordLen=nprx*npry
  ...
  include "constants.f90"
       ...
       if(allocated(p_sst)) deallocate(p_sst)
       if(allocated(p_sc))  deallocate(p_sc)
       if(allocated(p_aer_sc))  deallocate(p_aer_sc)
       ...
       if(CHEM_ASSIM == 1 .and. nspecies>0 ) allocate(p_sc(nprx,npry,nprz,nspecies))
       if(CHEM_ASSIM == 1 .and. nspecies_aer_in>0 ) allocate(p_aer_sc(nprx,npry,nprz,nspecies_aer_in))
