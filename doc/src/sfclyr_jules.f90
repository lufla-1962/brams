...
SUBROUTINE sfclyr_jules(mzp,mxp,myp,ia,iz,ja,jz,jdim,julesFile)
  ...
  USE prognostics,   ONLY : lai,gc,t_soil,tstar_tile,cs,snowdepth
  ...
  USE fluxes,        ONLY : emis_tile, land_albedo,  &
                            ...
                            anthrop_heat,            &   ! anthropgenic urban  heat source (W/m2) (land_pts,ntiles)
                            RADNET_TILE,             &   !   Surface net radiation on tiles ( W/m2) (land_pts,ntiles)
                            ftl_tile,                &   !   Surface FTL for land tiles    (land_pts,ntiles)
                            LE_TILE,                 &   !   Surface latent heat flux for land tiles  (land_pts,ntiles)
                            ...
                            tstar                        ! IN Gridbox Mean Surface Temperature (K)
...
!endif
   ...
   IF (start) THEN     !DSM --- Semelhante a time==0, mas serve tambem para o history
      ...
      INQUIRE(FILE=trim(julesFile),EXIST=tem_in)
      IF (tem_in) THEN
         OPEN (77,FILE=trim(julesFile), STATUS='old')
      ELSE
         WRITE(*,*)'Not found ',trim(julesFile),' control file.'
         STOP 
      ENDIF
  ...
  !.. call dumpVarAllLatLonk(basic_g(ng)%rv   ,'rv',201,0,0,1,mxp,1,myp,1,mzp,0.0,0.0)
  !.. call dumpVarAllLatLonK(basic_g(ng)%up   ,'up',201,0,0,1,mxp,1,myp,1,mzp,0.0,0.0)
  !.. call dumpVarAllLatLonK(basic_g(ng)%vp   ,'vp',201,0,0,1,mxp,1,myp,1,mzp,0.0,0.0)
  !.. call dumpVarAllLatLonK(basic_g(ng)%pi0  ,'pi0',201,0,0,1,mxp,1,myp,1,mzp,0.0,0.0)
  !.. call dumpVarAllLatLonK(basic_g(ng)%pp   ,'pp',201,0,0,1,mxp,1,myp,1,mzp,0.0,0.0)
  !.. call dumpVarAllLatLonK(basic_g(ng)%theta,'theta',201,0,0,1,mxp,1,myp,1,mzp,0.0,0.0)

  !.. call dumpVarAllLatLonK(grid_g(ng)%rtgt     ,'rtgt',201,0,0,1,mxp,1,myp,1,1,0.,0.)
  !.. call dumpVarAllLatLonK(radiate_g(ng)%cosz  ,'cosz',201,0,0,1,mxp,1,myp,1,1,0.,0.)
  !.. call dumpVarAllLatLonK(radiate_g(ng)%rshort,'rshort',201,0,0,1,mxp,1,myp,1,1,0.,0.)
  !.. call dumpVarAllLatLonK(radiate_g(ng)%rlong ,'rlong',201,0,0,1,mxp,1,myp,1,1,0.,0.)
  !.. call dumpVarAllLatLonK(cuparm_g(ng)%conprr ,'conprr',201,0,0,1,mxp,1,myp,1,1,0.,0.)
  !.. call dumpVarAllLatLonK(micro_g(ng)%pcpg    ,'pcpg',201,0,0,1,mxp,1,myp,1,1,0.,0.)
...
!               ,grid_g(ng)%glabasic_g(ng)%theta(k2,i+ia-1,j+ja-1)t(i+ia-1,j+ja-1), temp2(i+ia-1,j+ja-1)-273.15,basic_g(ng)%theta(2,i+ia-1,j+ja-1)  &
...
!ENDIF
   ...
   !RMF! ELSEIF (time==dtlong .or. mod(time,dtlong*fat_dtlong)==0) THEN
      ...
      !z= angulo zenital solar
      
      !AOD=profundidade optica dos aerossois no comprimento de onda 670 nm.
      !No rad_carma a AOD para 670 nm eh o aotl(:,15)
      
      !Fracao da radiacao difusa (diff_frac)
	...
	ENDIF
	 ...
	 rshort_diffuse(i+ia-1, j+ja-1)=radiate_g(ng)%rshort(i+ia-1, j+ja-1)*diff_frac
      ...
      DO l=1,land_pts
         j = ( land_index(l)-1 ) / row_length + 1
         i = land_index(l) - ( j-1 ) * row_length
!call dumpPlotPoint(l,i,j,land_index(l),row_length,0.,0.)
         IF (i<1 .or. i>mxJ .or. j<1 .or. j>myJ) THEN
            PRINT*, "ERRO... conversao incorreta de l para i,j -> l,i,j=",l,i,j
            STOP
         ENDIF
         ...
         DO n=1,ntiles
            ...
            IF (qual_ip(n) /= -999 .and. qual_ip(n) /= 1) THEN    
               ...
               !--- Output dos fluxos ---
               jules_g(ng)%anthrop_heatj(i+ia-1,j+ja-1,qual_ip(n))=anthrop_heat(l,n)
               jules_g(ng)%radnet_tilej(i+ia-1,j+ja-1,qual_ip(n))=RADNET_TILE(l,n)
               jules_g(ng)%ftl_tilej(i+ia-1,j+ja-1,qual_ip(n))=ftl_tile(l,n)
               jules_g(ng)%le_tilej(i+ia-1,j+ja-1,qual_ip(n))=LE_TILE(l,n)
               jules_g(ng)%htf_tilej(i+ia-1,j+ja-1,qual_ip(n))=surf_htf_tile(l,n)
               
               !--- Output para o GSI/3d-var ---
               jules_g(ng)%snowdepthj(i+ia-1,j+ja-1,qual_ip(n))=snowdepth(l,n)
               
            ENDIF

            !--- Output para o GSI/3d-var ---
            jules_g(ng)%fracj(n+1,i+ia-1,j+ja-1)=frac(l,n)
         ...
         jules_g(ng)%ht_fluxj(i+ia-1,j+ja-1)=SURF_HT_FLUX(i,j)
                  
         !--- Output para o GSI/3d-var ---
         jules_g(ng)%temp_surfj(i+ia-1,j+ja-1)=tstar(i,j)
...
!call dumpBug(ftl_1(i,j),latent_heat(i,j),alvl,tstar2,rstar2,leaf_g(ng)%patch_area(i+ia-1,j+ja-1,ip),i+ia-1,j+ja-1,ip)
      ENDDO
!call dumpVarAllLatLonk(turb_g(ng)%sflux_w,'sflux_w',623,0,0,1,mxp,1,myp,1,1,0.0,0.0) !Bad
  !RMF! ENDIF  !IF (time==0)
...
!.. subroutine dumpPlotPoint(l,i,j,land_index,row_length,begtime,endtime)
  !.. use node_mod, only:  &
       !.. nodei0, &
       !.. nodej0, mynum, nmachs
  !.. use mem_grid, only: time
    !.. implicit none
  !.. integer, intent(in) :: l,i,j,land_index,row_length
  ...
  !.. logical :: ft=.true.
    ...
    !.. if(ft) then
      !.. if(nmachs==1) then
        !.. open(unit=80,file='dumpDir/MapJ.1P1.dat')
        !.. open(unit=81,file='dumpDir/MapJ.1P2.dat')
      !.. elseif(nmachs>1)  then
        !.. open(unit=91,file='dumpDir/MapJ.2P1.dat')
        !.. open(unit=92,file='dumpDir/MapJ.2P2.dat')
      !.. endif
      !.. ft=.false.
    !.. endif

    !.. if(nmachs==1 .and. i+nodei0(mynum,1)<=31) &
      !.. write(80,fmt='(4(I10,1X))') l,land_index,i+nodei0(mynum,1),j+nodej0(mynum,1)
    !.. if(nmachs==1 .and. i+nodei0(mynum,1)>=30) &
      !.. write(81,fmt='(4(I10,1X))') l,land_index,i+nodei0(mynum,1),j+nodej0(mynum,1)
    !.. if(nmachs==2 .and. mynum==1) &
      !.. write(91,fmt='(4(I10,1X))') l,land_index,i+nodei0(mynum,1),j+nodej0(mynum,1)
    !.. if(nmachs==2 .and. mynum==2) &
      !.. write(92,fmt='(4(I10,1X))') l,land_index,i+nodei0(mynum,1),j+nodej0(mynum,1)
      
!.. end subroutine dumpPlotPoint

!.. subroutine dumpBug(cx,wtol,ustar2,tstar2,rstar2,patch_area,i,j,ip)
  !.. use node_mod, only:  &
       !.. nodei0, &
       !.. nodej0, mynum, nmachs
  !.. use mem_grid, only: time
  !.. integer, intent(in) :: i,j,ip
  !.. real,intent(in) :: cx,wtol,ustar2,patch_area,tstar2,rstar2
  
  !.. if(nmachs==1) then
    !.. write (70,fmt='(2(I2.2,1X),6(E16.8,1X))') i+nodei0(mynum,1),j+nodej0(mynum,1),cx,wtol,ustar2,tstar2,rstar2,patch_area
  !.. else
    !.. write (70+mynum,fmt='(2(I2.2,1X),6(E16.8,1X))') i+nodei0(mynum,1),j+nodej0(mynum,1),cx,wtol,ustar2,tstar2,rstar2,patch_area
  !.. endif

!.. end subroutine dumpBug
