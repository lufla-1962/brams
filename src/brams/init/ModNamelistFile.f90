...
contains
  ...
  subroutine ReadNamelistFile(oneNamelistFile)
    ...
    namelist /MODEL_GRIDS/                                               &
         ...
         domain_fname,                                                   &
         if_adap, ihtran, deltax, deltay, deltaz, dzrat, dzmax, fixLevels, &
         zz,      &
...
!--(DMK-CCATT-FIM)-----------------------------------------------------------
    ...
    namelist /MODEL_FILE_INFO/                                           &
         ...
         timeWindowIAU,ramp, tnudlat, tnudcent, tnudtop, znudtop, wt_nudge_grid, wt_nudge_uv,&
...
!--(DMK-CCATT-FIM)-----------------------------------------------------------
    ...
    namelist /MODEL_OPTIONS/ &
!--(DMK-CCATT-INI)-----------------------------------------------------------
         dyncore_flag, pd_or_mnt_constraint, &
                order_h, &
                order_v &
...
!--(DMK-CCATT-FIM)-----------------------------------------------------------
         ...
         iswrtyp, ilwrtyp,                                                 &
         raddatfn,                                                         &
         radfrq, radtun, lonrad, nnqparm, closure_type, nnshcu, confrq,            &
	 ...
	 epsil,gnparm,gnu,windfarm,wfFile,damModule,frqPrecip,damOutPrefix, &
      evaluate,evaluatePrefix
    ...
    namelist /ISAN_ISENTROPIC/ &
         ...
         gridwt, gobsep, gobrad, wvlnth, swvlnth, respon, icFileType, icPrefix, &
         ...
         final_longitude, z_max_level, dlimit, ulimit, scale_factor, &
...
100 format(/,'----------------------------NAMELIST VARIABLES-------',  &
    ...
    do ng = 1, oneNamelistFile%ngrids
       write(logUnit,"(1(a,i4))") &
            ' CONFIGURATION FOR GRID:',ng
       write(logUnit,"(5(a,i4))") &
    ...
    if (len_trim(oneNamelistFile%domain_fname) > 0) then
       write(logUnit, '("Domain File Name =",A80)') oneNamelistFile%domain_fname
    else
       write(logUnit, '("Using default domain decomposition")')
    endif


    write(logUnit,"(a)") " Closure type for Grell Parametrization is: "//oneNamelistFile%closure_type

    write(logUnit,999) (' ', oneNamelistFile%nnshcu(ng), ng=1, oneNamelistFile%ngrids)  ! For Shallow Cumulus Param.

    write(logUnit,103)(' ',oneNamelistFile%njnest(ng),oneNamelistFile%nknest(ng),oneNamelistFile%nnsttop(ng)  &
         ,oneNamelistFile%nnstbot(ng),oneNamelistFile%itoptflg(ng),ng=1,oneNamelistFile%ngrids)
    write(logUnit,104)(' ',oneNamelistFile%isstflg(ng),oneNamelistFile%ivegtflg(ng),oneNamelistFile%isoilflg(ng)  &
...
104 format(A1,'ISSTFLG=',I4,'   IVEGTFLG=',I4,'   ISOILFLG=',I4  &
    ...
    write(logUnit, "(' ')")
    write(logUnit,201)oneNamelistFile%ngrids,oneNamelistFile%nestz1,oneNamelistFile%nestz2,&
         oneNamelistFile%initial,oneNamelistFile%ioutput,oneNamelistFile%nudlat,&
         oneNamelistFile%if_adap,oneNamelistFile%applyIAU
201 format('  NGRIDS=',I4,'     NESTZ1=',I4,'     NESTZ2=',I4  &
         ,'    INITIAL=',I4,'    IOUTPUT=',I4,'     NUDLAT=',I4,'     IF_ADAP=',I4 &
	 ,     'applyIAU=', I4 )
    write(logUnit,FMT="('IPOS=',I4)") oneNamelistFile%ipos
    write(logUnit,202)oneNamelistFile%initfld,oneNamelistFile%ihtran,oneNamelistFile%nacoust,&
         oneNamelistFile%kwrite
202 format(' INITFLD=',I4,'     IHTRAN=',I4  &
         ,'    NACOUST=',I4,'     KWRITE=',I4)
    write(logUnit,203)oneNamelistFile%iupdsst,&
...
!--(DMK-CCATT-FIM)-----------------------------------------------------
         ,'     NSLCON=',I4,'       IBND=',I4)
    write(logUnit,204)oneNamelistFile%jbnd,oneNamelistFile%lsflg,oneNamelistFile%nfpt,&
...
204 format('    JBND=',I4,'      LSFLG=',I4,'       NFPT=',I4  &
    ...
    write(logUnit,205)oneNamelistFile%lonrad,oneNamelistFile%imonth1,oneNamelistFile%idate1,&
...
205 format('  LONRAD=',I4,'    IMONTH1=',I4,'     IDATE1=',I4  &
    ...
    write(logUnit,206)oneNamelistFile%nvgcon,oneNamelistFile%nplt,oneNamelistFile%ipsflg,&
         oneNamelistFile%itsflg,oneNamelistFile%irtsflg
206 format('  NVGCON=',I4,'       NPLT=',I4,'     IPSFLG=',I4  &
         ,'     ITSFLG=',I4,'    IRTSFLG=',I4)
    write(logUnit,207)oneNamelistFile%iusflg,oneNamelistFile%mkcoltab,oneNamelistFile%nzg,&
         oneNamelistFile%nzs,oneNamelistFile%iupdndvi
207 format('  IUSFLG=',I4,'   MKCOLTAB=',I4,'        NZG=',I4  &
         ,'        NZS=',I4,'   IUPDNDVI=',I4)
    write(logUnit,208)oneNamelistFile%npatch,oneNamelistFile%nvegpat
208 format('  NPATCH=',I4,'    NVEGPAT=',I4)

    write(logUnit,211)oneNamelistFile%mcphys_type,oneNamelistFile%level,&
         oneNamelistFile%icloud,oneNamelistFile%idriz
211 format('  MCPHYS_TYPE=',I4,'      LEVEL=',I4, '     ICLOUD=',I4 &
         ,'     IDRIZ=',I4)
    write(logUnit,212)oneNamelistFile%iccnlev,oneNamelistFile%irime,&
         oneNamelistFile%iplaws
212 format('  ICCNLEV=',I4,'     IRIME=',I4 &
         ,'     IPLAWS=',I4)

    write(logUnit,209)oneNamelistFile%irain,oneNamelistFile%ipris,oneNamelistFile%isnow,&
         oneNamelistFile%iaggr,oneNamelistFile%igraup
209 format('   IRAIN=',I4,'      IPRIS=',I4,'      ISNOW=',I4  &
         ,'      IAGGR=',I4,'     IGRAUP=',I4)

    write(logUnit,210)oneNamelistFile%ihail,oneNamelistFile%naddsc
210 format('   IHAIL=',I4,'     NADDSC=',I4 )

    write(logUnit, "(' ')")
    write(logUnit,301)(' ',oneNamelistFile%toptenh(ng),oneNamelistFile%toptwvl(ng),&
...
301 format(A1,'TOPTENH=',E12.5,'        TOPTWVL=',E12.5  &
    ...
    write(logUnit,302)(' ',oneNamelistFile%centlon(ng),oneNamelistFile%csx(ng),&
         oneNamelistFile%csz(ng),ng=1,oneNamelistFile%ngrids)
302 format(A1,'CENTLON=',E12.5,'            CSX=',E12.5  &
         ,'            CSZ=',E12.5,999(A1,/,E21.5,2E28.5))
    write(logUnit,303)(' ',oneNamelistFile%xkhkm(ng),oneNamelistFile%zkhkm(ng),&
         oneNamelistFile%akmin(ng),ng=1,oneNamelistFile%ngrids)
303 format(A1,'  XKHKM=',E12.5,'          ZKHKM=',E12.5  &
         ,'          AKMIN=',E12.5,999(A1,/,E21.5,2E28.5))
    write(logUnit,304)(' ',oneNamelistFile%gridu(ng),oneNamelistFile%gridv(ng),&
         ng=1,oneNamelistFile%ngrids)
304 format(A1,'  GRIDU=',E12.5,'          GRIDV=',E12.5  &
         ,999(A1,/,E21.5,E28.5))

!--(DMK-CCATT-INI)-----------------------------------------------------
    write(logUnit,305) oneNamelistFile%dyncore_flag, oneNamelistFile%advmnt,&
     oneNamelistFile%GhostZoneLength
305 format( "DYNCORE_FLAG=",I4,'  ADVMNT=',I4,'        GHOSTZONELENGTH=',I4)

write(logUnit,306) oneNamelistFile%pd_or_mnt_constraint, oneNamelistFile%order_h,&
 oneNamelistFile%order_v
306 format( "PD_OR_MNT_CONSTRAINT=",I1,'  ORDER_H=',I1,'        ORDER_V=',I1)
!--(DMK-CCATT-FIM)-----------------------------------------------------

    write(logUnit, "(' ')")
    write(logUnit,401)oneNamelistFile%timmax,oneNamelistFile%timstr,oneNamelistFile%frqhis
401 format('   TIMMAX=',E12.5,'         TIMSTR=',E12.5  &
         ,'         FRQHIS=',E12.5)
    write(logUnit,418)oneNamelistFile%frqanl,oneNamelistFile%frqprt,oneNamelistFile%timeWindowIAU&
    ,oneNamelistFile%ramp
418 format('   FRQANL=',E12.5,'         FRQPRT=',E12.5  &
         ,'        timeWindowIAU=',E12.5,'        RAMP=',E12.5)
    write(logUnit,402)oneNamelistFile%frqanl,oneNamelistFile%frqprt,oneNamelistFile%tnudlat
402 format('   FRQANL=',E12.5,'         FRQPRT=',E12.5  &
         ,'        TNUDLAT=',E12.5)
    write(logUnit,403)oneNamelistFile%tnudtop,oneNamelistFile%tnudcent,oneNamelistFile%znudtop
403 format('  TNUDTOP=',E12.5,'       TNUDCENT=',E12.5  &
         ,'        ZNUDTOP=',E12.5)
    write(logUnit,404)oneNamelistFile%dtlong,oneNamelistFile%deltax,oneNamelistFile%deltay
404 format('   DTLONG=',E12.5,'         DELTAX=',E12.5  &
         ,'         DELTAY=',E12.5)
    write(logUnit,405)oneNamelistFile%polelat,oneNamelistFile%polelon,oneNamelistFile%deltaz
405 format('  POLELAT=',E12.5,'        POLELON=',E12.5  &
         ,'         DELTAZ=',E12.5)
    write(logUnit,406)oneNamelistFile%dzrat,oneNamelistFile%dzmax
406 format('    DZRAT=',E12.5,'          DZMAX=',E12.5)
    
    write(logUnit,496)oneNamelistFile%fixLevels,oneNamelistFile%fixLevels
496 format('    FIXLEVELS=',I2,'          FIXLEVELS=',I2)

    write(logUnit,407)oneNamelistFile%cphas,oneNamelistFile%distim,oneNamelistFile%radfrq,oneNamelistFile%radtun
407 format('    CPHAS=',E12.5,'         DISTIM=',E12.5  &
         ,'         RADFRQ=',E12.5,'         RADTUN=',E12.5)
    write(logUnit,408)oneNamelistFile%confrq,oneNamelistFile%wcldbs
408 format('   CONFRQ=',E12.5,'         WCLDBS=',E12.5)

    write(logUnit,409)oneNamelistFile%shcufrq
409 format('   SHCUFRQ=',E12.5)  ! For Shallow Cumulus

    write(logUnit,410)oneNamelistFile%pctlcon,oneNamelistFile%zrough,oneNamelistFile%albedo
410 format('  PCTLCON=',E12.5,'         ZROUGH=',E12.5  &
         ,'         ALBEDO=',E12.5)
    write(logUnit,411)oneNamelistFile%seatmp,oneNamelistFile%dthcon,oneNamelistFile%drtcon
411 format('   SEATMP=',E12.5,'         DTHCON=',E12.5  &
         ,'         DRTCON=',E12.5)
    write(logUnit,412)oneNamelistFile%cparm,oneNamelistFile%rparm,oneNamelistFile%pparm
412 format('    CPARM=',E12.5,'          RPARM=',E12.5  &
         ,'          PPARM=',E12.5)
    write(logUnit,413)oneNamelistFile%sparm,oneNamelistFile%aparm,oneNamelistFile%gparm
413 format('    SPARM=',E12.5,'          APARM=',E12.5  &
         ,'          GPARM=',E12.5)
    write(logUnit,414)oneNamelistFile%hparm,oneNamelistFile%dparm
414 format('    HPARM=',E12.5, '    DPARM=',E12.5)
    write(logUnit,416)oneNamelistFile%cnparm,oneNamelistFile%gnparm,oneNamelistFile%epsil
416 format('    CNPARM=',E12.5, '    GNPARM=',E12.5, '    EPSIL=',E12.5)

    write(logUnit,415)oneNamelistFile%g3d_spread
...
!
!    write(*, "(' ')")
    write(logUnit,601)(' ',oneNamelistFile%itoptfn(m),m=1,oneNamelistFile%ngrids)
601 format(A1,'  ITOPTFN=',A40,999(A1,/,11X,A40))
    write(logUnit,602)(' ',oneNamelistFile%isstfn(m),m=1,oneNamelistFile%ngrids)
602 format(A1,'   ISSTFN=',A40,999(A1,/,11X,A40))
    write(logUnit,603)(' ',oneNamelistFile%ivegtfn(m),m=1,oneNamelistFile%ngrids)
603 format(A1,'  IVEGTFN=',A40,999(A1,/,11X,A40))
    write(logUnit,604)(' ',oneNamelistFile%isoilfn(m),m=1,oneNamelistFile%ngrids)
604 format(A1,'  ISOILFN=',A40,999(A1,/,11X,A40))
    write(logUnit,605)(' ',oneNamelistFile%ndvifn(m),m=1,oneNamelistFile%ngrids)
605 format(A1,'   NDVIFN=',A40,999(A1,/,11X,A40))
    write(logUnit,606) ' ',trim(oneNamelistFile%coltabfn)
606 format(A1,' COLTABFN=',A)
    write(logUnit,610) ' ',trim(oneNamelistFile%MapAOTFile)
610 format(A1,' MAPAOTFILE=',A)
    write(logUnit,611) ' ',trim(oneNamelistFile%julesin)
611 format(A1,' JULESIN=',A)
    write(logUnit,607) ' ',trim(oneNamelistFile%varfpfx)
607 format(A1,'  VARFPFX=',A)
    write(logUnit,608) ' ',trim(oneNamelistFile%sstfpfx)
608 format(A1,'  SSTFPFX=',A)
    write(logUnit,609) ' ',trim(oneNamelistFile%ndvifpfx)
609 format(A1,' NDVIFPFX=',A)

    !write(*, "(' ')")
    write(logUnit,701)oneNamelistFile%expnme
701 format('  EXPNME=',A40)
    write(logUnit,705)oneNamelistFile%runtype,oneNamelistFile%timeunit
705 format(' RUNTYPE=',A10,'      TIMEUNIT=',A3)
    !write(*, "(' ')")
    write(logUnit,702)oneNamelistFile%hfilin
702 format('  HFILIN=',A40)
    write(logUnit,703)oneNamelistFile%hfilout
703 format(' HFILOUT=',A40)
    write(logUnit,704)oneNamelistFile%afilout
704 format(' AFILOUT=',A40)
    write(logUnit,*) 'SOIL_MOIST = ', oneNamelistFile%soil_moist
    write(logUnit,*) 'SOIL_MOIST_FAIL = ', oneNamelistFile%soil_moist_fail
    write(logUnit,711) oneNamelistFile%usdata_in
711 format(' USDATA_IN =',A40)
    write(logUnit,712) oneNamelistFile%usmodel_in
712 format(' USMODEL_IN=',A40)
    write(logUnit,901)(' ',oneNamelistFile%slmstr(k),oneNamelistFile%slz(k),&
         oneNamelistFile%stgoff(k),k=1,oneNamelistFile%nzg)
901 format(A1,' SLMSTR=',F6.2,'      SLZ=',F7.3,'      STGOFF=',F8.2  &
         ,999(A1,/,F15.2,F17.3,F21.2))
    write(logUnit,1001)(oneNamelistFile%zz(k),k=1,oneNamelistFile%nnzp(1))
1001 format('ZZ=',8F9.1,/,(F12.1,7F9.1))

    write(logUnit,1101)(oneNamelistFile%nstratz1(k),k=1,oneNamelistFile%nnzp(1))
1101 format(/,'NSTRATZ1=',(t9,23i3) )

    write(logUnit,1201)(oneNamelistFile%nstratz2(k),k=1,oneNamelistFile%nnzp(1))
1201 format(/,'NSTRATZ2=',(t9,23i3) )

    write(logUnit,1301)(oneNamelistFile%gnu(k),k=1,7)
1301 format(/,'GNU=',(t9,7f5.2))

    write(logUnit,1302) oneNamelistFile%windfarm
1302 format('     WINDFARM=',I1)
     write(logUnit,1303) oneNamelistFile%wfFile
1303 format('     WFFILE=',A40)
    write(logUnit,1304) oneNamelistFile%damModule
1304 format('     DAMMODULE=',I1)
    write(logUnit,1305) oneNamelistFile%frqPrecip
1305 format('     FRQPRECIP=',F18.2)
    write(logUnit,1306) trim(oneNamelistFile%damOutPrefix)
1306 format('     damOutPrefix=',A40)
    write(logUnit,1307) trim(oneNamelistFile%evaluatePrefix)
1307 format('     evaluatePrefix=',A40)
    write(logUnit,1308) oneNamelistFile%evaluate
1308 format('     evaluate=',I1)

    write(logUnit,1500) oneNamelistFile%fileNameIAU
1500 format('     fileNameIAU=',A40)


    !New rad carma
    if (oneNamelistFile%iswrtyp==4 .or. oneNamelistFile%ilwrtyp==4) then
       write(logUnit,FMT='("RADDATFN = ",A80)')  trim(oneNamelistFile%raddatfn)
    endif

    !write(*, "('==============================================================')")
    if (oneNamelistFile%teb_spm/=1) then
       write(logUnit,FMT='(A,I2)') 'TEB_SPM module not activated: TEB_SPM=', oneNamelistFile%teb_spm
    else
       write(logUnit,FMT='(A,I2)') 'TEB_SPM module activated: TEB_SPM=', oneNamelistFile%teb_spm
    endif
    !write(*, "('==============================================================')")

!--(DMK-CCATT-INI)-----------------------------------------------------------
    if (oneNamelistFile%ccatt == 1) then
       write(logUnit,FMT='(A,I2)') 'CCATT module activated:'
       write(logUnit,FMT='(3x,A,I2)') 'CHEMISTRY= ', oneNamelistFile%chemistry
       write(logUnit,"(3x,a)") "SPLIT_METHOD= "//trim(oneNamelistFile%split_method)
       write(logUnit,FMT='(3x,A,F8.2)') 'CHEM_TIMESTEP= ', oneNamelistFile%chem_timestep
       write(logUnit,FMT='(3x,A,I2)') 'CHEMISTRY_AQ= ', oneNamelistFile%chemistry_aq
       write(logUnit,FMT='(3x,A,I2)') 'CHEM_ASSIM= ', oneNamelistFile%chem_assim
       write(logUnit,"(3x,a)") "SRCMAPFN= "//trim(oneNamelistFile%srcmapfn)
       write(logUnit,FMT='(3x,A,I2)') 'RECYCLE_TRACERS= ', oneNamelistFile%recycle_tracers
       write(logUnit,"(3x,a)") "DEF_PROC_SRC ="//trim(oneNamelistFile%def_proc_src)
       write(logUnit,FMT="(3x,A,4i3)")'DIUR_CYCLE=',(oneNamelistFile%diur_cycle(k),k=1,nsrc)
!   format((t12,23i3) )
       write(logUnit,FMT='(3x,A,I2)') 'NA_EXTRA2D= ', oneNamelistFile%na_extra2d
       write(logUnit,FMT='(3x,A,I2)') 'NA_EXTRA3D= ', oneNamelistFile%na_extra3d
       write(logUnit,FMT='(3x,A,I2)') 'PLUMERISE= ', oneNamelistFile%plumerise
       write(logUnit,FMT='(3x,A,F8.2)') 'PRFRQ= ', oneNamelistFile%prfrq
       write(logUnit,FMT='(3x,A,I2)') 'VOLCANOES= ', oneNamelistFile%volcanoes
       write(logUnit,FMT='(3x,A,I2)') 'AEROSOL= ', oneNamelistFile%aerosol
       !if(oneNamelistFile%aerosol == 2) then
         write(logUnit,FMT='(3x,A,F8.2)') 'AER_TIMESTEP= ', oneNamelistFile%aer_timestep
         !write(*,FMT='(3x,A,I2)')'AER_ASSIM=', oneNamelistFile%aer_assim
         !write(*,FMT='(3x,A,I2)') 'MECH= ', oneNamelistFile%mech
       !endif
       !write(*, "(' ')")
    else
       write(logUnit,FMT='(A,I2)') 'CCATT module NOT activated - CCATT=',oneNamelistFile%ccatt
    endif
    ! dump POST
    IF(oneNamelistFile%ipos > 0 ) then
     write(logUnit,"(a)") "POST file prefix is "//trim(oneNamelistFile%gprefix)
     write(logUnit,"(3x,a,i3,a)") "POST will write ", oneNamelistFile%nvp, " fields: "
     ...
     do k = 1, oneNamelistFile%nvp
       ...
       if (lenLine+lenThis > 80) then
          write(logUnit, "(' ')")
          write(logUnit,"(3x,a)", advance="no") trim(oneNamelistFile%vp(k))
          lenLine = lenThis + 1
    ...
    ELSE
     write(logUnit,"(a,I2)") "POST processing is turned off: IPOS=",oneNamelistFile%ipos
    ENDIF

    write(logUnit,"(/,'----------------------------NAMELIST VARIABLES ENDS--',&
         &'------------------------',/)")
