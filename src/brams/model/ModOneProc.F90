...
!XXXsrf    integer :: iau_phase
    ...
    dtSet_firstTime=.true.
     ...
     if(len_trim(tmpdir)==0) then
       if (mynum==master_num) iErrNumber=dumpMessage(c_tty,c_yes,header,version,c_fatal, &
           "Environment variable for temporary directory non existent, empty or not exported."//&
			     "Please export TMPDIR to an existent directory, e.g. ~> export TMPDIR=./tmp")
     endif
	...
	!---------------------------------------------------------------------------
	!  If using an Intel compiler, you probably need to change the next line to:
#ifdef INTEL
    ...
    IF ( .NOT. dirExist)  THEN
      IF (mynum==master_num) iErrNumber=dumpMessage(c_tty,c_yes,header,version,c_fatal, &
           "TMPDIR environment variable assigned to an non existent directory."//&
           "Please export TMPDIR to an existent directory, e.g. ~> export TMPDIR=./tmp")
    ENDIF
    ...
    if (oneParallelEnvironment%mchnum==oneParallelEnvironment%master_num) then
      ...
      write (*,fmt='(A)') '           '//BRAMS_version
#ifdef color
      write (*,fmt='(A)') c_noColor
       write(c0,"(i12)") oneParallelEnvironment%nmachs
       write(OUTPUT_UNIT,"(a)") "+---------------- Information about submission ------------------------"
       write(OUTPUT_UNIT,"(a)") "! Run Type: "//c_lightYellow//trim(oneNamelistFile%runtype)//c_noColor
       write(OUTPUT_UNIT,"(a)") "! Input namelist filename: "//c_lightYellow//trim(oneNamelistFile%fileName)//c_noColor
       write(OUTPUT_UNIT,"(a)") "! Parallel execution using "//c_lightYellow//trim(adjustl(c0))//c_noColor//" processes"
#else
      write(c0,"(i12)") oneParallelEnvironment%nmachs
      write(OUTPUT_UNIT,"(a)") "+---------------- Information about submission ------------------------"
      write(OUTPUT_UNIT,"(a)") "! Run Type: "//trim(oneNamelistFile%runtype)
      write(OUTPUT_UNIT,"(a)") "! Input namelist filename: "//trim(oneNamelistFile%fileName)
      write(OUTPUT_UNIT,"(a)") "! Parallel execution using "//trim(adjustl(c0))//" processes"
#endif
       write(OUTPUT_UNIT,"(a)") "+----------------------------------------------------------------------"
       if(trim(oneNamelistFile%runtype)=='MAKESFC' .and. oneParallelEnvironment%nmachs>1)&
       iErrNumber=dumpMessage(c_tty,c_yes,header,modelVersion,c_fatal,'For run type '// &
    ...
    !print *,'LFR: ',oneParallelEnvironment%master_num
    ! store namelist info at old places
    !iErrNumber=debug("Apenas um teste!","LFR","ModOneProc.f90","OneProc",c_continue,oneParallelEnvironment%mchnum,1,'Banana')
    !iErrNumber=debug("Apenas um teste!","LFR","ModOneProc.f90","OneProc",c_continue,oneParallelEnvironment%mchnum,1,(/'Banana','Pera  ','Maca  '/))
    !iErrNumber=debug("Apenas um teste!","LFR","ModOneProc.f90","OneProc",c_kill,oneParallelEnvironment%mchnum,1)
