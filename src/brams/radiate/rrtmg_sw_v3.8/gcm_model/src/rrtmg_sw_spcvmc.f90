...
! exponential for low tau
               ...
               zdbtc(jk) = zdbtmc
               !write(*,fmt='("LFR$ztdbtc: ",I6,1X,2(E18.6,1X))') &
              !    jk,zdbtc(jk),ztdbtc(jk)
               !LFR
               if(ztdbtc(jk)>=0.100000-289) then
                 ztdbtc(jk+1)=ztdbtc(jk)
               else
...
! exponential for low tau
               ...
               !write(*,fmt='("LFR$ztdbt: ",I6,1X,2(E18.6,1X))') &
              !    jk,zdbt(jk),ztdbt(jk)
               if(ztdbt(jk)>=0.100000-289) then
                 ztdbt(jk+1) = ztdbt(jk)
               else
