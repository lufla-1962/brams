...
contains
  ...
  subroutine soilmoistureinit(n1, n2, n3, mzg, mzs, npat, ifm,   &
    ...
    real, allocatable  :: api_us(:,:,:), prlat(:,:), prlon(:,:), usdum(:),api_temp(:,:,:), tempdum(:)
    real               :: dif_time, dummy
	  ...
	  allocate( soilM_GFS(nlon,nlat,n4us),  soilT_GFS(nlon,nlat,n4us))
	     ...
	     write(*,*) 'Filename    : ',usdata(1:len_trim(usdata)-3)//'bin'
	      ...
	      open(2, status='old', form='unformatted', access='direct', &
	        	      recl=    nlat*nlon, file=usdata(1:len_trim(usdata)))
...
!write(86,fmt='(6(I3.3,1X),4(F5.1,1X))') i,j,i1,j1,ic,jc,globalglon(i,j),prlon(ic,1),globalglat(i,j),prlat(1,jc)
             if (ic>=0 .and. jc>=0) then
                dlonr = 0.5*(globalglon(nnxp(ifm)-1,j) - globalglon(1,j))/&
                float(nnxp(ifm)-1)
                dlatr = 0.5*(globalglat(i,nnyp(ifm)-1) - globalglat(i,1))/&
                float(nnyp(ifm)-1)
!write(87,fmt='(2(I3.3,1X),2(F8.4,1X))') i,j,dlonr,dlatr
...
!write(88,fmt='(6(I4.4,1X),2(F5.1,1X),2(F18.6,1X))') i,j,ii,jj,k,ipat,prlon(ii,jj),prlat(ii,jj),api_us(ii,jj,k),usdum(k)
                            enddo
                         endif
                      enddo
                   enddo
